...
$(function () {
    ...
    //商品名
    $('#ContentPlaceHolder2_TextBox1').keyup(function (event) {
        ZhengZeMsg(/^.{0,50}$/, $('#ContentPlaceHolder2_TextBox1').val(), box1, ".A-nameMsg1", ".A-nameMsg2");//调用验证
    });
    //花语
    $('#ContentPlaceHolder2_TextBox2').keyup(function (event) {
        ZhengZeMsg(/^.{0,100}$/, $('#ContentPlaceHolder2_TextBox2').val(), box2, ".A-LMsg1", ".A-LMsg2");//调用验证
    });
    //花材
    $('#ContentPlaceHolder2_TextBox3').keyup(function (event) {
        ZhengZeMsg(/^.{0,100}$/, $('#ContentPlaceHolder2_TextBox3').val(), box3, ".A-materialsMsg1", ".A-materialsMsg2");//调用验证
    });
    //包装
    $('#ContentPlaceHolder2_TextBox5').keyup(function (event) {
        ZhengZeMsg(/^.{0,50}$/, $('#ContentPlaceHolder2_TextBox5').val(), box5, ".A-packMsg1", ".A-packMsg2");//调用验证
    });
    //介绍
    $('#ContentPlaceHolder2_TextBox4').keyup(function (event) {
        ZhengZeMsg(/^.{0,200}$/, $('#ContentPlaceHolder2_TextBox4').val(), box4, ".A-presentationMsg1", ".A-presentationMsg2");//调用验证
    });
    //单价
    $('#ContentPlaceHolder2_TextBox6').keyup(function (event) {
        ZhengZeMsg(/^(\+)?\d+(\.\d+)?$/, $('#ContentPlaceHolder2_TextBox6').val(), box6, ".A-priceMsg1", ".A-priceMsg2");//调用验证
    });
    //数量
    $('#ContentPlaceHolder2_TextBox7').keyup(function (event) {
        ZhengZeMsg(/^(\+)?\d+(\d+)?$/, $('#ContentPlaceHolder2_TextBox7').val(), box7, ".A-numMsg1", ".A-numMsg2");//调用验证
    });
...
function preImg(source, targetId) {
    ...
    imgPre.src = url;
    if ($("#ContentPlaceHolder2_Image1")[0].src != "") {
...
function ButClick() {
    let name = $('#ContentPlaceHolder2_TextBox1'); // 商品名
    let Language = $('#ContentPlaceHolder2_TextBox2'); // 花语
    let materials = $('#ContentPlaceHolder2_TextBox3'); // 花材
    let pack = $('#ContentPlaceHolder2_TextBox5'); // 包装
    let presentation = $('#ContentPlaceHolder2_TextBox4'); // 介绍
    let price = $('#ContentPlaceHolder2_TextBox6'); // 单价
    let num = $('#ContentPlaceHolder2_TextBox7'); // 数量
    ...
    //展示图片
    let imgb = true;
    let url = $("#ContentPlaceHolder2_Image1")[0].src;
    if (url == "") {
        $(".A-imgMsg1").css("display", "block");
        imgb = false;
    }
